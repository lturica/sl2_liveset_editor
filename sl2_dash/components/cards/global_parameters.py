...
liveset_name_card = dbc.Card([
    ...
    dbc.CardBody([
        dbc.Col([
            ...
            dbc.Input(id=GlobalSettings.LIVE_SET_NAME, value="New Live Set", type="text", valid=True)
            ], 
            width="auto"),
        ...
        html.Div([
            dbc.Col([
                ...
                dbc.Input(id=GlobalSettings.PATCH_NAME, value="CUSTOM-PATCH-1", type="text", valid=True)
                ], 
                width="auto")
            ],
        ...
        html.Div([
                dbc.Row(dbc.Button("Duplicate Patch", id=GlobalSettings.PATCH_DUPLICATE_BUTTON, color="primary", className="mb-2",)),
                dbc.Row(dbc.Button("Delete Patch", id=GlobalSettings.PATCH_DELETE_BUTTON, color="primary",))
                ],
        ...
        dbc.CardBody(
            dbc.Col([
                dbc.Col([
                    ...
                    dbc.Input(id=GlobalSettings.MEMO, value="", type="text", disabled=True)
                ], width="auto"),
                dbc.Col([
                    dbc.Label(["Format Rev:", make_tooltip("format_rev_tt")]),
                    dbc.Input(id=GlobalSettings.LIVE_SET_FORMAT_REV, value="0001", type="text", disabled=True)
                ], width="auto"),
                dbc.Col([
                    dbc.Label(["Device:", make_tooltip("device_tt")]),
                    dbc.Input(id=GlobalSettings.LIVE_SET_DEVICE, value="SL-2", type="text", disabled=True)
                ], width="auto")
            ])
        )
    ])
...
patch_selection_card = dbc.Card([  
        ...
        dbc.CardBody(
            dbc.Col([                
                dbc.Col([
                    dbc.Label(["Select Other Patch:",make_tooltip("select_patch_tt")]),
...
global_tts = ['ls_name_tt','patch_name_tt','memo_tt','format_rev_tt','device_tt','select_patch_tt']

global_cards = [ liveset_name_card, patch_selection_card,global_pars_card]

global_outputs = [Output(lsp, "value", allow_duplicate=True) for lsp in GlobalSettings.global_list()]
