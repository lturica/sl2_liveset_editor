...
beat_card = dbc.Card([
        dbc.CardHeader(f"Beat Parameters",),
        dbc.CardBody([
        dbc.Row([
            dbc.Col([
                dbc.Label(["Param 1", make_tooltip(beat_tts["Param 1"])]),
                dcc.Slider(id=beat_params["Param 1"], min=0, max=1, value=0, disabled=False, **SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Param 2", make_tooltip(beat_tts["Param 2"])]),
                dcc.Slider(id=beat_params["Param 2"], min=0, max=1, value=1, disabled=False, **SLIDER_ARGS)
            ],width="auto"),
        ])
    ])],
    id=f"beat_params"
