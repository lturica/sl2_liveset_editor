...
channel_1_states = [slicer_c1_state[(24*i+4) :(24*(i+1)+4)] for i in range(5)]+[
                    phaser_c1_state,
                    flanger_c1_state,
                    tremolo_c1_state,
                    overtone_c1_state]
channel_2_outputs = [slicer_c2_outputs[(24*i+4) :(24*(i+1)+4)] for i in range(5)]+[
                     phaser_c2_outputs,
                     flanger_c2_outputs,
                     tremolo_c2_outputs,
                     overtone_c2_outputs]
...
def register_copy_c1_c2_button_callbacks(app:dash.Dash)->None:
    ...
    for i in range(len(copy_button_inputs)):
        @app.callback(Output(ChannelCopy.DOUBLE_CLICK_TIMESTAMP,'data',allow_duplicate=True),
                    *channel_2_outputs[i],
        ...
        def _duplicate_c1_to_c2(n_clicks,timestamp,*args):
            ...
            if (current_time-old_time).total_seconds()>0.5: # if less than 0.5 seconds, consider double click
