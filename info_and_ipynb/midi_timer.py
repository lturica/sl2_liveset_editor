...
def choose_output(port_hint: str | None) -> mido.ports.BaseOutput:
    ...
    if not outs:
        print("No MIDI outputs found. Is your interface connected and recognized by the OS?", file=sys.stderr)
        sys.exit(1)
    ...
    if port_hint:
        ...
        print(f"Couldn't find an output containing '{port_hint}'. Available outputs:", file=sys.stderr)
        for name in outs:
        ...
        if len(outs) == 1:
            ...
            print("Multiple MIDI outputs available. Use --port to pick one. Available outputs:", file=sys.stderr)
            for name in outs:
                print(f"  - {name}", file=sys.stderr)
            sys.exit(1)
...
def run_clock(out: mido.ports.BaseOutput, bpm: float):
    ...
    print(f"Running MIDI Clock at {bpm:.2f} BPM (interval ~ {interval*1000:.3f} ms between ticks).")
    # Start
...
def run_countdown(out: mido.ports.BaseOutput, seconds: int, channel: int, note: int, cc: int, tick_ms: int):
    """
    Every second:
    ...
    remaining = seconds
    print(f"Countdown: {seconds}s on MIDI channel {channel+1}. Tick note={note}, CC={cc}. Press Ctrl+C to cancel.")
    ...
    try:
        while remaining >= 0:
            ...
            if now >= next_sec:
                ...
                out.send(mido.Message('control_change', channel=channel, control=cc, value=cc_val))

                if remaining > 0:
                    # Tick note
                    out.send(mido.Message('note_on', channel=channel, note=note, velocity=80))
                    ...
                    out.send(mido.Message('note_on', channel=channel, note=note+7, velocity=90))
                    time.sleep(0.2)
                    out.send(mido.Message('note_off', channel=channel, note=note, velocity=0))
                    out.send(mido.Message('note_off', channel=channel, note=note+7, velocity=0))
                    print("Time's up!")
                    break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send MIDI Clock or a MIDI-based countdown timer.")
    sub = parser.add_subparsers(dest='mode', required=True)

    p_clock = sub.add_parser('clock', help="Send MIDI Clock at a given BPM.")
    p_clock.add_argument('--bpm', type=float, default=120.0, help='Beats per minute (default: 120.0)')
    p_clock.add_argument('--port', type=str, help='Substring of the MIDI output port name to open')

    p_count = sub.add_parser('countdown', help="Send a 1-second tick and CC with remaining seconds.")
    p_count.add_argument('seconds', type=int, help='How many seconds to count down')
    p_count.add_argument('--port', type=str, help='Substring of the MIDI output port name to open')
    p_count.add_argument('--channel', type=int, default=0, help='MIDI channel 1–16 (enter 1–16; default: 1)')
    p_count.add_argument('--note', type=int, default=60, help='Tick note number (default: 60=C4)')
    p_count.add_argument('--cc', type=int, default=20, help='CC number to send remaining seconds on (default: 20)')
    p_count.add_argument('--tick-ms', type=int, default=60, help='Length of tick note in ms (default: 60)')
