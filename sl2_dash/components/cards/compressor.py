...
compressor_card = dbc.Card([
    ...
    [
        dbc.Row([
            ...
            dbc.Col([
                dbc.Label(["Sustain",make_tooltip(compressor_tts["Param 1"])]),
                dcc.Slider(id=compressor_params["Param 1"],min=0,max=100,value=50,**SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Attack", make_tooltip(compressor_tts["Param 2"])]),
                dcc.Slider(id=compressor_params["Param 2"], min=0, max=100, value=50,**SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Level", make_tooltip(compressor_tts["Param 3"])]),
                dcc.Slider(id=compressor_params["Param 3"], min=0, max=100, value=60,**SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Tone", make_tooltip(compressor_tts["Param 4"])]),
                dcc.Slider(id=compressor_params["Param 4"], min=0, max=100, value=50,**SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Ratio", make_tooltip(compressor_tts["Param 5"])]),
                dcc.Slider(id=compressor_params["Param 5"], min=0, max=30, value=12,**SLIDER_ARGS)
            ],width="auto"),
            dbc.Col([
                dbc.Label(["Direct Mix", make_tooltip(compressor_tts["Param 6"])]),
                dcc.Slider(id=compressor_params["Param 6"], min=0, max=30, value=0, **SLIDER_ARGS)
            ], width="auto")
        ])
    ])],
...
compressor_outputs = [Output(cm, "value",allow_duplicate=True) for cm in compressor_params.values()]
