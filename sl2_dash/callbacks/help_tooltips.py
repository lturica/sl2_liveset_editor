...
def register_tooltip_modals(app:dash.Dash)->None:
    ...
    for name in channel_tooltips:
        if '_enable_tt' in name : # through a miracle, the tooltips are 'slicer_enable_c1_tt' and 'phaser_c1_enable_tt', so this chooses the right ones
            @app.callback(Output(f"generic_enable_tt_modal","is_open",allow_duplicate=True),
                    Input(name,"n_clicks"),
                    prevent_initial_call=True)
            def _handle_enable_tooltips_noslicer(_):
                return modal_open()
        else:
            @app.callback(Output(f"{cmatch.sub('_',name)}_modal","is_open",allow_duplicate=True),
                        Input(name,"n_clicks"),
                        prevent_initial_call=True )
    ...
    for name in mono_fx_tooltips:
        ...
        # tooltips for all other parameters
            ...
            @app.callback(Output(f"{name}_modal","is_open"),
                        Input(name,"n_clicks"),
                        prevent_initial_call=True)
