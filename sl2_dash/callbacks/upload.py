...
def register_upload_callback(app:dash.Dash, ):
    ...
    @app.callback(
        ALL_OUTPUTS + [
            ...
            Output(CURRENT_PATCH_ID,'data',allow_duplicate=True) # set the current patch to first one when upload is successful
        ],
    ...
    def _upload_and_update_ui(contents,n_default_clicks:int,default_tsl_name:str):
        ...
        if ctx.triggered_id is None:
            return [dash.no_update] * ALL_OUTPUTS_LEN + [dash.no_update, show_err, show_success,current_patch_id]
