...
SLIDER_LABELS = [("step_length", 'Step Length'),
...
def create_slicer_2channels_card():
    ...
    def make_channel_parameter_card(cnum, p_names, tt_names ):
        top_params_card = dbc.Card([
            dbc.CardHeader( f"Slicer Channel {cnum} Parameters",),
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                    ...
                    dbc.Col([
                        dbc.Label(["Time-Step Number:", make_tooltip(tt_names["Step Num"])]),
                        dbc.Select(id=p_names["Step Num"],
                                options=[{'label': '8 Steps', 'value': 0}, 
    ...
    slider_tab_card = dbc.Card([
        dbc.CardBody([
            dbc.Row(dbc.Label(["Time-Step Parameter Arrays:", make_tooltip(tt_names_1["Parameter Arrays"])])),
            dbc.Row([
                dbc.Tabs(
                    [
                        dbc.Tab(
                            children=[
                                dbc.Label('Channel 1',  style={"marginTop": "10px"} ),
                                dbc.Row(sliders_dict_1[slider_type]), 
                                html.Hr(),
                                dbc.Label('Channel 2',style={"marginTop": "-20px"} ),
                                dbc.Row(sliders_dict_2[slider_type]),
                                html.Div(
                                    dbc.Row(
                                        dbc.Button("Double-click to copy Channel 1 to Channel 2", 
                                                       id=ChannelCopy.SLICER[slider_type], color="primary", n_clicks=0)
                                        ),
                 ...
                 id=f"slicer_tabs", active_tab=f"slicer_step_length")
            ], )
        ],className="h-100")], 
    id=f"slicer_sliders")
    ...
    return slicer_cards, tooltips1,  outputs1, states1, sliders1,  tooltips2,  outputs2, states2, sliders2


slicer_c1_card, slicer_c1_tts,  slicer_c1_outputs, slicer_c1_state, slicer_c1_slider_ids, slicer_c2_tts,  slicer_c2_outputs, slicer_c2_state, slicer_c2_slider_ids= create_slicer_2channels_card()
