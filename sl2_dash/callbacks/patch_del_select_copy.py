...
def register_patch_modify_callbacks(app: dash.Dash):
    ...
    @app.callback(
        Output(GlobalSettings.CONFIRM_DELETE_MODAL, "is_open", allow_duplicate=True),
    ...
    @app.callback(
        Output(GlobalSettings.PATCH_SELECT_DROPDOWN, "options", allow_duplicate=True),
    ...
    def _generate_patch_select_list(json_str: str):
        ...
        return [
            {"label": patchName, "value": i} for i, patchName in enumerate(patch_names)
        ]
    ...
    def _handle_patch_buttons(
        n_load,
        n_copy: int,
        n_delete: int,
        json_str: str,
        current_patch_id: int,  # dropdown_value:int,
        *state_values,
    ):
        ...
        if trig == GlobalSettings.PATCH_SELECT_DROPDOWN:
            ...
            ui_values = liveset_to_ui_outputs(
                live_set, patch_id=new_patch_id
            )  # load new patch to ui
            new_json = json.dumps(
                live_set.dict(), separators=(",", ":")
            )  # json encode liveset
            select_toast = True
        ...
        # --- DUPLICATE PATCH ---
            ...
            live_set.data[0][-1].paramSet.com.string = (
                live_set.data[0][-1].paramSet.com.string[:12] + "COPY"
            )  # remove the last bits of the string and replace name with cpy
            live_set.data[0][-1].paramSet._storage["PATCH%COM"] = live_set.data[0][
                -1
            ].paramSet.com
